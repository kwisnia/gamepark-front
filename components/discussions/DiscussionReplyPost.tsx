...
import RemoveDiscussionPostDialog from "./RemoveDiscussionPostDialog";
...
interface DiscussionPostProps {
  ...
  originalPostRef?: HTMLDivElement | null;
}

const DiscussionReplyPost = forwardRef<HTMLDivElement, DiscussionPostProps>(
  ({ post, discussion, mutate, originalPost, originalPostRef }, ref) => {
    ...
    return (
      <Box bg="gray.700" rounded="md" p={3} ref={ref}>
          ...
          {post.originalPostID ? (
            <Box>
              <Box
                fontWeight="bold"
                cursor="pointer"
                onClick={() => originalPostRef?.scrollIntoView()}
              >
                Reply to:{" "}
                {originalPost ? originalPost.user.displayName : "Deleted post"}
              </Box>
            </Box>
          ) : null}
        </Box>
